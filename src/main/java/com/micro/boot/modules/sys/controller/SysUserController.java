...
public class SysUserController extends AbstractController {
	 ...
	 */
	@RequestMapping("/info")
	public ReturnMapInfo info(){
		return ReturnMapInfo.ok().put("user", getUser());
	}
	...
	public ReturnMapInfo password(String password, String newPassword){
		...
		if(count == 0){
			return ReturnMapInfo.error("原密码不正确");
		}
	...
	public ReturnMapInfo save(@RequestBody SysUserEntity user){
	...
	public ReturnMapInfo update(@RequestBody SysUserEntity user){
	...
	public ReturnMapInfo delete(@RequestBody Long[] userIds){
		if(ArrayUtils.contains(userIds, 1L)){
			return ReturnMapInfo.error("系统管理员不能删除");
		}
		
		if(ArrayUtils.contains(userIds, getUserId())){
			return ReturnMapInfo.error("当前用户不能删除");
		}
