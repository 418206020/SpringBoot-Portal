...
import com.micro.boot.modules.sys.service.SysUserTokenService;
...
public class SysLoginController extends AbstractController {
	...
	public Map<String, Object> login(String username, String password, String captcha)throws IOException {
		...
		if(!captcha.equalsIgnoreCase(kaptcha)){
			return ReturnMapInfo.error("验证码不正确");
		}
		...
		if(user == null || !user.getPassword().equals(new Sha256Hash(password, user.getSalt()).toHex())) {
			return ReturnMapInfo.error("账号或密码不正确");
		}

		//账号锁定
		if(user.getStatus() == 0){
			return ReturnMapInfo.error("账号已被锁定,请联系管理员");
		}

		//生成token，并保存到数据库
		ReturnMapInfo returnMapInfo = sysUserTokenService.createToken(user.getUserId());
